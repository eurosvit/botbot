...
app=Flask(__name__); app.config["SECRET_KEY"]=os.getenv("SECRET_KEY","change")
try: migrate()
...
def last_orders():
    s=get_session()
    rows=s.execute(text("SELECT id,created_at,order_id,amount_uah,brand,utm_campaign,status,shipped_at FROM orders ORDER BY id DESC LIMIT 10")).mappings().all()
...
def webhook_salesdrive():
    ...
    try:
        ...
        def amount_of(pp):
            for keys in (("total_uah",),("amount_uah",),("grand_total_uah",),("total",),("amount",),("order","amount")):
                node=pp; ok=True
                for k in keys:
        ...
        if amt is not None:
            if isinstance(shipped,(int,float)):
                s.execute(text("INSERT INTO orders(order_id,amount_uah,brand,utm_campaign,status,shipped_at) VALUES(:o,:a,:b,:c,:s,to_timestamp(:t))"),{"o":order_id,"a":amt,"b":brand,"c":camp,"s":status,"t":shipped})
            else:
                s.execute(text("INSERT INTO orders(order_id,amount_uah,brand,utm_campaign,status,shipped_at) VALUES(:o,:a,:b,:c,:s,:t)"),{"o":order_id,"a":amt,"b":brand,"c":camp,"s":status,"t":shipped})
        s.commit()
        try:
            msg=(f"🧾 <b>Нова подія із SalesDrive</b>\nID: <code>{order_id}</code>\nСума: <b>{int(amt) if amt is not None else '—'}</b>\nutm_source: {source}\nutm_campaign: {camp}")
            Telegram().send(msg)
        except Exception: logging.exception("tg_notify_failed")
        return ("ok",200)
    except Exception:
