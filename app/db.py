...
def _coerce(u):
    """
    Перетворює URL схему для PostgreSQL.
    """
    p = urlparse(u)
...
def _ssl(u):
    """
    Додає параметр sslmode=require, якщо він відсутній.
    """
    p = urlparse(u)
...
def get_engine():
    """
    Повертає об'єкт двигуна SQLAlchemy.
    """
...
def get_session():
    """
    Створює нову сесію для роботи з базою даних.
    """
    if _Session is None:
        get_engine()
    return _Session()
...
def migrate():
    ...
    with eng.begin() as c:
        ...
        # Додаткові колонки для таблиці orders
        for sql in (
        ...
        # Таблиця для статистики реклами
        c.execute(text("""
        ...
        # Таблиця для щоденних звітів
        c.execute(text("""
            CREATE TABLE IF NOT EXISTS daily_reports (
                day DATE PRIMARY KEY,
                orders_count INTEGER,
                orders_ads_count INTEGER,
                sales_total NUMERIC,
                avg_check NUMERIC,
                ad_cost NUMERIC,
                manager_cost NUMERIC,
                avg_margin NUMERIC,
                gross_profit NUMERIC,
                net_profit NUMERIC,
                processing_orders_count INTEGER,
                processing_amount NUMERIC,
                real_sales_count INTEGER,
                real_sales_amount NUMERIC,
                real_avg_check NUMERIC,
                real_gross_profit NUMERIC,
                clarity_json JSONB
            )
        """))
