# Daily report with SQLAlchemy text() fix
import os, json
...
def format_report(day, f):
    ...
    lines=[
        ...
        f"✅ Чистий прибуток: {f['net_profit']:.0f} {cur} {profit}",
...
def save_daily(day, f, clarity):
    ...
    INSERT INTO daily_reports(
        ...
        :d,:oc,:oac,:st,:ac,:ad,:mgr,:am,:gp,:np,
        :pc,:pa,:rc,:ra,:rac,:rgp,:cj
    )
    ...
    """), {
        "d":day,
        "oc":int(f['new_orders_count']),
        "oac":int(f['orders_ads_count']),
        "st":float(f['real_sales_amount']),
        "ac":float(f['real_avg_check']),
        "ad":float(f['ad_cost']),
        "mgr":float(f['manager_cost']),
        "am":float(f['avg_margin']),
        "gp":float(f['gross_profit']),
        "np":float(f['net_profit']),
        "pc":int(f['processing_orders_count']),
        "pa":float(f['processing_amount']),
        "rc":int(f['real_sales_count']),
        "ra":float(f['real_sales_amount']),
        "rac":float(f['real_avg_check']),
        "rgp":float(f['gross_profit']),
        "cj":json.dumps(clarity) if clarity else None
    })
    s.commit()
...
def main():
    ...
    clarity=fetch_clarity_json()
    txt=format_report(day,f)
    save_daily(day,f,clarity)
    Telegram().send(txt)
    print(txt)
