...
def format_report(day, f):
    cur=os.getenv("CURRENCY","UAH"); shipped=(f['real_sales_count']/f['new_orders_count']*100) if f['new_orders_count'] else 0
    flag="✅" if shipped>=60 else "⚠️" if shipped>=40 else "🚨"
    lines=[f"📊 Щоденний звіт ({day})",
...
def main():
    ...
    s=get_session()
    s.execute(text("""INSERT INTO daily_reports(day,orders_count,orders_ads_count,sales_total,avg_check,ad_cost,manager_cost,avg_margin,gross_profit,net_profit,
    ...
    """), {"d":day,"oc":int(f['new_orders_count']),"oac":int(f['orders_ads_count']),"st":float(f['real_sales_amount']),"ac":float(f['real_avg_check']),
             "ad":float(f['ad_cost']),"mgr":float(f['manager_cost']),"am":float(f['avg_margin']),"gp":float(f['gross_profit']),"np":float(f['net_profit']),
             "pc":int(f['processing_orders_count']),"pa":float(f['processing_amount']),"rc":int(f['real_sales_count']),"ra":float(f['real_sales_amount']),
             "rac":float(f['real_avg_check']),"rgp":float(f['gross_profit']),"cj":None})
    s.commit()
    txt=format_report(day,f); Telegram().send(txt); print(txt)
